...
{
	Crank_Signal_VDAC8_Init();
	Crank_Signal_VDAC8_SetSpeed(Crank_Signal_HIGHSPEED);
	Crank_Signal_VDAC8_SetRange(Crank_Signal_DAC_RANGE);

	#if(Crank_Signal_DAC_MODE == Crank_Signal_CURRENT_MODE)
		Crank_Signal_IDAC8_SetPolarity(Crank_Signal_DAC_POL);
	#endif /* Crank_Signal_DAC_MODE == Crank_Signal_CURRENT_MODE */
	...
	Crank_Signal_Wave1TD = CyDmaTdAllocate();
	Crank_Signal_Wave2TD = CyDmaTdAllocate();
	
	/* Initialize waveform pointers  */
	Crank_Signal_Wave1Setup(Crank_Signal_wave1, Crank_Signal_WAVE1_LENGTH) ;
	Crank_Signal_Wave2Setup(Crank_Signal_wave2, Crank_Signal_WAVE2_LENGTH) ;
	
	/* Initialize the internal clock if one present  */
	#if defined(Crank_Signal_DacClk_PHASE)
	   Crank_Signal_DacClk_SetPhase(Crank_Signal_CLK_PHASE_0nS);
	#endif /* defined(Crank_Signal_DacClk_PHASE) */
}


/*******************************************************************************
* Function Name: Crank_Signal_Enable
********************************************************************************
*  
* Summary: 
...
void Crank_Signal_Enable(void) 
{
	...
	*/
	(void)CyDmaChEnable(Crank_Signal_Wave1Chan, 1u);
	(void)CyDmaChEnable(Crank_Signal_Wave2Chan, 1u);
	
	/* set the initial value */
	Crank_Signal_SetValue(0u);
	
	#if(Crank_Signal_CLOCK_SRC == Crank_Signal_CLOCK_INT)  	
	   Crank_Signal_DacClk_Start();
	#endif /* Crank_Signal_CLOCK_SRC == Crank_Signal_CLOCK_INT */
}


/*******************************************************************************
* Function Name: Crank_Signal_Start
********************************************************************************
*
* Summary:
...
void Crank_Signal_Start(void) 
{
	...
	if(Crank_Signal_initVar == 0u)
	{
		Crank_Signal_Init();
		Crank_Signal_initVar = 1u;
	}
...
/*******************************************************************************
* Function Name: Crank_Signal_StartEx
********************************************************************************
*
* Summary:
...
{
	Crank_Signal_Wave1Setup(wavePtr1, sampleSize1);
	Crank_Signal_Wave2Setup(wavePtr2, sampleSize2);
	Crank_Signal_Start();
}


/*******************************************************************************
* Function Name: Crank_Signal_Stop
********************************************************************************
*
* Summary:
...
void Crank_Signal_Stop(void) 
{
	...
	#if(Crank_Signal_CLOCK_SRC == Crank_Signal_CLOCK_INT)  	
	   Crank_Signal_DacClk_Stop();
	#endif /* Crank_Signal_CLOCK_SRC == Crank_Signal_CLOCK_INT */
	
	/* Disble DMA channels */
	(void)CyDmaChDisable(Crank_Signal_Wave1Chan);
	(void)CyDmaChDisable(Crank_Signal_Wave2Chan);
...
/*******************************************************************************
* Function Name: Crank_Signal_Wave1Setup
********************************************************************************
*
* Summary:
...

{
		...
		memoryType = (Crank_Signal_HI16FLASHPTR == HI16(wavePtr)) ? HI16(CYDEV_FLS_BASE) : HI16(CYDEV_SRAM_BASE);
		
		Crank_Signal_Wave1Chan = Crank_Signal_Wave1_DMA_DmaInitialize(
		Crank_Signal_Wave1_DMA_BYTES_PER_BURST, Crank_Signal_Wave1_DMA_REQUEST_PER_BURST,
		memoryType, HI16(CYDEV_PERIPH_BASE));
	#else /* PSoC 5 */
		Crank_Signal_Wave1Chan = Crank_Signal_Wave1_DMA_DmaInitialize(
		Crank_Signal_Wave1_DMA_BYTES_PER_BURST, Crank_Signal_Wave1_DMA_REQUEST_PER_BURST,
		HI16(wavePtr), HI16(Crank_Signal_DAC8__D));
	#endif /* CY_PSOC3 */
	...
	*/
	(void)CyDmaTdSetConfiguration(Crank_Signal_Wave1TD, sampleSize, Crank_Signal_Wave1TD, 
                                    (uint8)CY_DMA_TD_INC_SRC_ADR | (uint8)Crank_Signal_Wave1_DMA__TD_TERMOUT_EN); 
	
	/* Set the TD source and destination address */
	(void)CyDmaTdSetAddress(Crank_Signal_Wave1TD, LO16((uint32)wavePtr), LO16(Crank_Signal_DAC8__D));
	
	/* Associate the TD with the channel */
	(void)CyDmaChSetInitialTd(Crank_Signal_Wave1Chan, Crank_Signal_Wave1TD);
}


/*******************************************************************************
* Function Name: Crank_Signal_Wave2Setup
********************************************************************************
*
* Summary:
 ...
 
{
		...
		Crank_Signal_Wave2Chan = Crank_Signal_Wave2_DMA_DmaInitialize(
		Crank_Signal_Wave2_DMA_BYTES_PER_BURST, Crank_Signal_Wave2_DMA_REQUEST_PER_BURST,
		memoryType, HI16(CYDEV_PERIPH_BASE));
	#else /* PSoC 5 */
		Crank_Signal_Wave2Chan = Crank_Signal_Wave2_DMA_DmaInitialize(
		Crank_Signal_Wave2_DMA_BYTES_PER_BURST, Crank_Signal_Wave2_DMA_REQUEST_PER_BURST,
	...
	* Increment the source address, but not the destination address. 
	*/
	(void)CyDmaTdSetConfiguration(Crank_Signal_Wave2TD, sampleSize, Crank_Signal_Wave2TD, 
                                    (uint8)CY_DMA_TD_INC_SRC_ADR | (uint8)Crank_Signal_Wave2_DMA__TD_TERMOUT_EN); 
	
	/* Set the TD source and destination address */
	(void)CyDmaTdSetAddress(Crank_Signal_Wave2TD, LO16((uint32)wavePtr), LO16(Crank_Signal_DAC8__D));
	
	/* Associate the TD with the channel */
	(void)CyDmaChSetInitialTd(Crank_Signal_Wave2Chan, Crank_Signal_Wave2TD);
}
